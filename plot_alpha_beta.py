...
def plot_alpha_beta(alpha_hat, beta):
    ...
    #plt.plot(alpha_hat.cpu().numpy(), color='r', label='alpha_hat')
    color1 = 'tab:red'
    ax1.set_ylabel(r'$\beta_t$ schedule', color=color1, fontsize=18)
    ...
    ax2.set_ylabel(r'$\bar{\alpha}_t$', color=color2, fontsize=18)
    #ax2.plot(torch.sqrt(alpha_hat).cpu().numpy(), color=color2, label='sqrt(alpha_hat)')
    ax2.plot(alpha_hat.cpu().numpy(), color=color2, label='alpha_bar')
